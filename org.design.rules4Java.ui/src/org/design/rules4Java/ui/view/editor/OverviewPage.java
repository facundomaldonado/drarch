...
public class OverviewPage extends DrarchFormPage {
	...
	protected void createFormContent(IManagedForm managedForm) {
		...
		helpImage.addHyperlinkListener(new IHyperlinkListener() {
			...
			public void linkActivated(HyperlinkEvent e) {
				...
				//no se por que no muestra la ayuda
			}
		...
		if (model!=null){
			text = toolkit.createText(sectionClient, String.valueOf(model.getRules().size()));
		}
		else text = toolkit.createText(sectionClient, String.valueOf(0));
		td = new TableWrapData(TableWrapData.FILL_GRAB);
		td.grabHorizontal=true;
		td.colspan=2;
		text.setLayoutData(td);
		...
		section = toolkit.createSection(form.getBody(),	Section.DESCRIPTION|Section.EXPANDED |Section.FOCUS_TITLE);
		 ...
		 *  Query Section 
		 */
		Section subSection= toolkit.createSection(ruleSectionClient,Section.EXPANDED | Section.DESCRIPTION | Section.TREE_NODE);
		...
		queryText= toolkit.createText(querySectionClient,"",SWT.MULTI|SWT.BORDER|SWT.V_SCROLL| SWT.H_SCROLL);
		TextLData.verticalSpan= 5;
		queryText.setLayoutData(TextLData);
		queryText.setEditable(false);
		final Button setQueryButton;
		Button editQueryButton =toolkit.createButton(querySectionClient,"edit",SWT.NONE);
		...
		Section varSubSection= toolkit.createSection(querySectionClient,Section.EXPANDED | Section.DESCRIPTION | Section.TREE_NODE);
		...
		varsText=new org.eclipse.swt.widgets.List(varsSectionClient,SWT.BORDER|SWT.V_SCROLL);
		TextLData.verticalSpan = 15;
		varsText.setLayoutData(TextLData);
			
		Button varsButton =toolkit.createButton(varsSectionClient,"get Vars",SWT.NONE);
		varsButton.addSelectionListener(new SelectionListener(){
			@SuppressWarnings("unchecked")
			public void widgetSelected(SelectionEvent e) {
				QueryVarsDialog queryVarsDialog=new QueryVarsDialog(currentRule.getQuery().getQueryString());
				queryVarsDialog.open();
				List vars=(List) queryVarsDialog.getVars();
				if (vars.size()>0){
					String[] values={};
		 ...
		 *  Suggest Section 
		 */
		subSection= toolkit.createSection(ruleSectionClient,Section.EXPANDED | Section.DESCRIPTION | Section.TREE_NODE);
		...
		suggestTemplateText= toolkit.createText(suggestSectionClient,"", SWT.MULTI | SWT.BORDER | SWT.V_SCROLL | SWT.H_SCROLL);
		suggestTemplateText.setLayoutData(TextLData);
		suggestTemplateText.setEditable(false);
		
		final Button setSuggestButton; 
		Button editSuggestButton =toolkit.createButton(suggestSectionClient,"edit",SWT.NONE);
			
		setSuggestButton =toolkit.createButton(suggestSectionClient,"set ",SWT.NONE);
		...
		factsTemplateText= new org.eclipse.swt.widgets.List(factsSectionClient,SWT.MULTI|SWT.BORDER|SWT.V_SCROLL );
		...
		Button addFactButton =toolkit.createButton(factsSectionClient," New  ",SWT.NONE);
		...
		deleteFactButton =toolkit.createButton(factsSectionClient,"Remove",SWT.NONE);
		...
		treeViewer.addSelectionChangedListener(new ISelectionChangedListener(){
			...
			public void selectionChanged(SelectionChangedEvent event) {
				...
				if (node!=null)
					if ((!node.getName().equals("Rules"))&&(node.getName().startsWith("Rule :"))) 
					{	
		...
		Button editRule = toolkit.createButton(sectionClient, "Edit Rule", SWT.PUSH);
		gd = new GridData(GridData.CENTER);
		 ...
		 * remove button
		 */
		Button removeRule = toolkit.createButton(sectionClient, "Remove ", SWT.PUSH);
		gd = new GridData(GridData.CENTER);
		...
		removeRule.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if(node.getValue() instanceof Rule){
					...
					MessageDialog.openError(form.getShell(), "Error: Deleting Rule", "Select a rule please");
				}
			}
		});
		section.setClient(sectionClient);
	...
	protected void setValues() {
		...
		if (query!=null){
			...
			List varList= query.getChosenVars();
			if(varList!=null){
				Iterator varI=currentRule.getQuery().getChosenVars().iterator();
				while (varI.hasNext()) {
					Var var= (Var)varI.next();
					varsText.add(var.getVarText());
				}
			}
		}
		suggestTemplateText.setText(currentRule.getSuggestTemplate());//by default ==""
		FactSet currentRuleFactSet=currentRule.getFactSet();
		if(currentRuleFactSet!=null){
			List factList=currentRule.getFactSet().getFactTemplates();
			if(factList!=null){
				Iterator factI=factList.iterator();
				while (factI.hasNext()) {
					Fact fact = (Fact) factI.next();
					factsTemplateText.add(fact.getFactText());
				}
			}
		}
	}
	...
	private TreeParent getRoot() {
		...
		if (getModel()!=null){
			Iterator i=getModel().getRules().iterator();
			int index=0;
			while(i.hasNext()){
				Rule r=(Rule)i.next();
