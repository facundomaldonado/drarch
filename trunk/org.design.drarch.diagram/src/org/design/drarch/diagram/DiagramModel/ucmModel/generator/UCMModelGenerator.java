...
public class UCMModelGenerator{
	...
	Map<String,List<String>> comp_class=new HashMap(); //componentName--list<className>
	Map<String,List<String>> class_meth=new HashMap(); //className    --list<methodName>
	
	private Map<String,String> resp_comp=new HashMap();//responsabilityName, componentName
	...
	private List getNodesFromBase(){
		...
		while (responsibilityQueryResult.hasMoreElements()){
			...
			while (executionQueryResult.hasMoreElements()){
				...
				NodeInfo node=new NodeInfo(responsibilityName,execId,className,methodName);
	...
	private void getComponentRoles(){
		List responsibilities=getResponsibilites();
		for(Iterator i=responsibilities.iterator();i.hasNext();){
			String resp=(String)i.next();
	...
	private void getPathNodes(List nodes) {
		...
		for(Iterator i=nodes.iterator(); i.hasNext();){
			...
			lastNode.addComment(" - " + className.substring(index+1, className.length()) + "->" + node.methodName + "\n");
			lastAlternativeNode.addComment(" - " + className.substring(index+1, className.length()) + "->" + node.methodName + "\n");
			
			PathNode actualAlternativeNode = UcmModelFactory.eINSTANCE.createPathNode();
	...
	public void make(){
		getNodesFromBase();
		List nodes = getNodes();//ordenada por id de ejecucion
		processNodes(nodes);
		model = UcmModelFactory.eINSTANCE.createUCMModel();
	...
	public List getResponsibilites() {
	...
	public Component getComponent(String responsibility) {
		...
		{	//recupero el nombre del componente asociado de la base de datos
			Query q=RuleModelFactory.eINSTANCE.createQuery();
			q.setQueryString(LogQueryFactory.getInstance().createComponentResponsibilityAssociationQuery(responsibility));
		...
		if(  !componentName.equals("")||componentName!=null){
			Component component= DiagramManager.getInstance().getCurrentModel().getComponent(componentName);
			if(component!=null)
				return  component;
		}
		return null;
