...
public class MethodInvocationVisitor extends ASTVisitor {
	...
	public static final Logger logger = Logger.getLogger(MethodInvocationVisitor.class.getName());
	 ...
	 * 
	 * @param theSourceLocation The full method name (package.class.method).
	 */
	...
	public boolean visit(MethodInvocation node) {
		if (node.getExpression() instanceof SimpleName) {
			...
			if (fullClassName == null) {
				fullClassName = getPackageName(sourceLocation) + "." + className;
			}
			SimpleName methodName = node.getName();
			String fact = "invocationMethod(" + sourceLocation + ", " + 
					fullClassName + "." + methodName.getFullyQualifiedName() + ").";	
	  		try {
				DrarchApplication.INSTANCE.getDrarchEngine().getKnowledgeBase().addFact(fact);
	...
	private String getPackageName(String fullMethodName) {
		String fullClasName = fullMethodName.substring(0, fullMethodName.lastIndexOf("."));
		String fullPackageName = fullClasName.substring(0, fullClasName.lastIndexOf("."));
	 ...
	 * @param fact The fact without the arguments.
	 */
	private void addArguments(MethodInvocation node, String fact) {
		for (Object object : node.arguments()) {
			...
			String newFact = fact.substring(0, fact.indexOf(").")) + ", " +
					argument + ").";
			try {
				DrarchApplication.INSTANCE.getDrarchEngine().getKnowledgeBase().addFact(newFact);
			} catch (DrarchEngineModelException e) {
				e.printStackTrace();
			}
		}
	...
	/**
	 * Get the full name of the specified class (e.g. package.class).
	 * Inspect the <code>ASTNode</code> to get the package name.
	 * 
	 * @param node The <code>ASTNode</code>.
	 * @param className The Name of the class.
	 * @return Returns a full qualified name of the class name specified. Returns null
	 * 		   if the import declaration don't have the specified class.
	 */
