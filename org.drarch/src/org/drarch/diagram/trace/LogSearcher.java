package org.drarch.diagram.trace;
...
import org.drarch.diagram.DiagramModel.ucmModel.generator.NodeInfo;
...
public class LogSearcher {
	...
	public Tag getRoot(TraceLog traceLog) {
		...
		for (Iterator iter = traceLog.getContexts().iterator(); iter.hasNext();) {
			...
			Tag contextTag = TagUtil.createChildTag(contexts,
					context.getName(), true);
			int contextIndex = 0;
			for (Iterator tagIter = context.getTags().iterator(); tagIter
					.hasNext();) {
				Tag tag = (Tag) tagIter.next();
				TagUtil.addChildTag(contextTag, tag, Integer
						.toString(contextIndex++), false);
			}
		}
	...
	public List<Responsibility> getResponsibilities() {
		...
		for (int i = 0; i < responsabilidades.length; i++) {
			...
			LogNode[] executions = ((TagLogNode) responsabilidades[i])
					.getChildrens();
			for (int k = 0; k < executions.length; k++) {
				LogNode[] mater_info = ((TagLogNode) executions[k])
						.getChildrens();
