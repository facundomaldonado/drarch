...
public class UCMDiagrams {
	...
	private ComponentRole getRole(String roleName) {
		...
		for (int i = 0; i < diagram.getMap().getComponentRoles().size(); i++) {
			componentRole = (ComponentRole) diagram.getMap().getComponentRoles().get(i);
			if (componentRole.getName().equals(roleName))
	...
	public void createComponentRole(String componentName){
		...
		ComponentRole componentRole = CoremodelFactory.eINSTANCE.createComponentRole();
	...
	public void createPath(Vector pathResponsibility){
		...
		SimplePathNode startNode = CoremodelFactory.eINSTANCE.createSimplePathNode();
		...
		for (int i = 0; i < pathResponsibility.size(); i++) {
			...
			previusNode = addResponsibilityNode(path, (String) peer.get(0), previusNode);
	...
	@SuppressWarnings("unchecked")
	private SimplePathNode addResponsibilityNode(Path path, String responsibilityName, SimplePathNode previusNode) {
		//esto es provisorio
			...
			if (diagram.getChildren().get(i) instanceof NodeVisualModel){
				...
				for (int j = 0; j < nodeVisualModel.getChildren().size(); j++) {
					if (nodeVisualModel.getChildren().get(j) instanceof NodeVisualModel){
						NodeVisualModel responsibilityNodeVisualModel =(NodeVisualModel) nodeVisualModel.getChildren().get(j);
						if (responsibilityNodeVisualModel.getSemanticModel() instanceof SimplePathNode){
							SimplePathNode simplePathNode = (SimplePathNode) responsibilityNodeVisualModel.getSemanticModel();
	...
	private SimplePathNode getNode(Path path, String nodeName) {
		for (int i = 0; i < path.getNodes().size(); i++) {
			...
			if (path.getNodes().get(i) instanceof SimplePathNode){
				...
				if (simplePathNode.getName().equals(nodeName))
	...
	private void drawNodes(Path path) {
		...
		for (int i = 0; i < (path.getNodes().size()); i++) {
			NodeVisualModel nodeVisual  = EditormodelFactory.eINSTANCE.createNodeVisualModel();
	...
	private void drawConnections(Path path){
		...
		for (int i = 0; i < (path.getNodes().size()); i++) {
			...
			if (nodeModel instanceof ResponsibilityNode){
				ComponentRole componentRoleModel = (ComponentRole) ((ResponsibilityNode) nodeModel).getRole();
	...
	private NodeVisualModel getNodeVisual(String id) {
		for (int i = 0; i < diagram.getChildren().size(); i++) {
			if (diagram.getChildren().get(i) instanceof NodeVisualModel){
				NodeVisualModel nodeVisual = (NodeVisualModel) diagram.getChildren().get(i);
				if (nodeVisual.getSemanticModel() instanceof SimplePathNode){
						if (((SimplePathNode) nodeVisual.getSemanticModel() ).getID().equals(id))
							return nodeVisual;
				}else {
					if (nodeVisual.getSemanticModel() instanceof ComponentRole){
							if (((ComponentRole) nodeVisual.getSemanticModel() ).getID().equals(id))
								return nodeVisual;
					}
				}
			}
	...
	public void createNote(Path path) {
		...
		for (int i = 0; i < path.getNodes().size(); i++) {
			...
			if (!(simplePathNode.getName().equals("StartNode")||(simplePathNode.getName().equals("EndNode")))){
				...
				NodeVisualModel responsibilityVisualModel = getResponsibilityNode(simplePathNode.getID());
				
				ConnectionVisualModel connection  = EditormodelFactory.eINSTANCE.createConnectionVisualModel();
	...
	/**
	 * Algoritmo simple para organizar
	 * los componentes en el editor
	 *
	 */
