...
public class ComponentsDiagram {
	...
	public ComponentsDiagram(String name) {
		...
		ComponentDiagram componentDiagram = ComponentmodelFactory.eINSTANCE.createComponentDiagram();
	...
	public void createComponent(String name){
		if (!existComponent(name)){
			ComponentModel component = CoremodelFactory.eINSTANCE.createComponentModel();
			component.setName(name);
			coreModel.getComponents().add(component);
			
			//Crea el Componente Visual
			NodeVisualModel nodeVisualModel  = EditormodelFactory.eINSTANCE.createNodeVisualModel();
			nodeVisualModel.setSemanticModel(component);
			diagram.getChildren().add(nodeVisualModel);
		}
		else
			if (!existComponentVisual(name)){
				//Crea el Componente Visual
				NodeVisualModel nodeVisualModel  = EditormodelFactory.eINSTANCE.createNodeVisualModel();
	...
	private boolean existComponent(String name) {
		for (int i = 0; i < coreModel.getComponents().size(); i++) {
			ComponentModel componentModel = (ComponentModel) coreModel.getComponents().get(i);
	...
	/**
	 * crea una relacion entre dos componentes. La relacion se
	 * crea en el modelo y en el diagrama. 
	 * @param target
	 * @param source
	 */
	@SuppressWarnings("unchecked")
	public void createRelationship(String target, String source, String stereotypeName){
		if (existComponent(target) && existComponent(source) && !source.equals(target)){
			ComponentModel componentTarget = getComponentModel(target);
			ComponentModel componentSource = getComponentModel(source);
			
			Relationship relationship = CoremodelFactory.eINSTANCE.createRelationship();
			...
			ConnectionVisualModel connectionVisualModel = EditormodelFactory.eINSTANCE.createConnectionVisualModel();
	...
	/**
	 * Busca en el diagrama visual el nodo que tenga como
	 * nombre el valor de name.
	 * @param name
	 * @return
	 */
	private NodeVisualModel getComponentNode(String name) {
		for (int i = 0; i < diagram.getChildren().size(); i++) {
			...
			if (object instanceof NodeVisualModel){
				...
				ComponentModel componentModel = (ComponentModel) nodeVisualModel.getSemanticModel();
				if (componentModel.getName().equals(name))
	...
	/**
	 * Busca en el modelo un componente que tenga como 
	 * nombre el valor de name
	 * @param name
	 * @return
	 */
	...
	/**
	 * Crea una interface en el puerto especificado 
	 * dentro del componente especificado. 
	 */
	@SuppressWarnings("unchecked")
	public void createProvidedInterface(String interfaceName, String portName, String componentName){
		@SuppressWarnings("unused") boolean hasRequireds = false;
		ComponentModel componentModel = getComponentModel(componentName);
		EList portList = componentModel.getOwnedPorts();
		InterfaceModel providedInterface = CoremodelFactory.eINSTANCE.createInterfaceModel();
		...
		NodeVisualModel componentNode = getComponentNode(componentName);
		NodeVisualModel providedInterfaceVisual = EditormodelFactory.eINSTANCE.createNodeVisualModel();
	...
	/**
	 * Retorna la interface proveedora del 
	 * puerto especificado.
	 */
	...
	/**
	 * Retorna la interface requerida del
	 * puerto especificado
	...
	@SuppressWarnings("unchecked")
	public void createRequiredsInterface(String interfaceName, String portName, String componentName){
		boolean hasProvideds = false;
		
		ComponentModel componentModel = getComponentModel(componentName);
		EList portList = componentModel.getOwnedPorts();
		InterfaceModel requiredInterface = CoremodelFactory.eINSTANCE.createInterfaceModel();
		...
		//Crea la interface en el puerto para visualizarla
		NodeVisualModel componentNode = getComponentNode(componentName);
		NodeVisualModel requiredInterfaceVisual = EditormodelFactory.eINSTANCE.createNodeVisualModel();
	...
	@SuppressWarnings("unchecked")
	public void createInterfaceLink(String sourceComponent, String targetComponent, 
									String portProvided, String portRequired, 
									String interfaceProvided, String interfaceRequired){
		if (existComponent(sourceComponent) && existComponent(targetComponent) && !targetComponent.equals(sourceComponent)){
			
			InterfaceModel sourceInterface = getInterfaceProvided(sourceComponent, portProvided);
			InterfaceModel targetInterface = getInterfaceRequired(targetComponent, portRequired);
			
			InterfaceLink interfaceLink = CoremodelFactory.eINSTANCE.createInterfaceLink();
			...
			NodeVisualModel providedsInterface = getInterface(portSourceVisual, interfaceProvided);
			NodeVisualModel requiredsInterface = getInterface(portTargetVisual, interfaceRequired);
			
			ConnectionVisualModel linkModel = EditormodelFactory.eINSTANCE.createConnectionVisualModel();
	...
	private NodeVisualModel getInterface(VisualModel portVisual, String interfaceName) {
		EList interfaces = portVisual.getChildren();
		for (int i = 0; i < interfaces.size(); i++) {
			NodeVisualModel interfaceVisual = (NodeVisualModel) interfaces.get(i);
			InterfaceModel interfaceModel = (InterfaceModel) interfaceVisual.getSemanticModel();
	...
	public VisualModel getPort(String componentName, String portName){
		...
		for (int i = 0; i < ports.size(); i++) {
			...
			if (port.getName().endsWith(portName))
				return portSourceVisual;
		}
		return null;
	}
	@SuppressWarnings("unchecked")
	...
	@SuppressWarnings("unchecked")
	public void createStereotypeInComponent(String componentName, String stereotypeName){
	...
	public void setScope(String componentName, Vector association){
		if (association != null){
			if (association.size() > 0){
				ComponentModel component = getComponentModel(componentName);
				PatternMapping pm = (PatternMapping) component.getExtendedData(MappingPlugin.SYMBOLIC_NAME, "mapping");
				String packageNames = "";
				String classNames = "";
...
//				}
				...
				for (int i = 0; i < association.size(); i++) {
					...
					if (!packageNames.contains(dObject.getPackageName())){
						if (packageNames.equals(""))
							...
							packageNames = packageNames + separator + escape(dObject.getPackageName());
					}
					if (classNames.equals(""))
						classNames = "@" + escape(dObject.getClassName());
					else
						classNames = classNames + separator + "@" + escape(dObject.getClassName());
					
					if (javaFileNames.equals(""))
						javaFileNames = escape(dObject.getJavaFileDescriptor());
					else
						javaFileNames = javaFileNames + separator + escape(dObject.getJavaFileDescriptor());
				}	
	...
	public Responsibility getResponsibility(String name){
		for (int i = 0; i < coreModel.getResponsibilities().size(); i++) {
			Responsibility res = (Responsibility) coreModel.getResponsibilities().get(i);
	...
	public void createResponsibility(String name) {
		...
		Responsibility responsibility = CoremodelFactory.eINSTANCE.createResponsibility();
		...
		JavalogTraceInferenceStrategy inferenceStrategy = JavalogtraceFactory.eINSTANCE.createJavalogTraceInferenceStrategy();
		inferenceStrategy.setPrologCode(
		"executionState('Faulty') :- texecutionTag(T), isExitError(T). " +
		...
		//traceStrategy.setResponsibility(responsibility);
		responsibility.putExtendedData("org.isistan.flabot.engine", "stateDeterminationStrategy", traceStrategy);
	...
	@SuppressWarnings("unchecked")
	public void registryResponsibility(String componentName, String responsibilityName) {
		Responsibility responsibility = getResponsibility(responsibilityName);
		
		getComponentModel(componentName).getFeatures().add(getResponsibility(responsibilityName));
		...
		responsibility.putExtendedData(LauncherPlugin.SYMBOLIC_NAME, "logFilter", logFilter);
	}
	...
	public void mappingResponsibility(String responsibilityName, Vector mapping){
		...
		if (mapping != null){
			if (mapping.size() > 0){
				Responsibility responsibility = getResponsibility(responsibilityName);
				PatternMapping pm = (PatternMapping) responsibility.getExtendedData(MappingPlugin.SYMBOLIC_NAME, "mapping");
				String packageNames = "";
				String classNames = "";
				String javaFileNames = "";
				String behavior = "";
...
//				}
				...
				for (int i = 0; i < mapping.size(); i++) {
					...
					if (behavior.equals(""))
						...
						behavior = behavior + separator + escape(dObject.getBehaviorString());
					/*
					 * src.Player#&lt;init>()
					 * |
					 * src.Player#getItems():int
					 * |
				...
				responsibility.putExtendedData(MappingPlugin.SYMBOLIC_NAME, "mapping", pm);
			}
		}
	}
	...
	public void organizeLayout() {
		...
		int y = 0;
		for (int i = 0; i < diagram.getChildren().size(); i++) {
			NodeVisualModel nodeVisualModel = (NodeVisualModel) diagram.getChildren().get(i);
