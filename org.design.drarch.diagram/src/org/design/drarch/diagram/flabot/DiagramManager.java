...
public class DiagramManager implements IDiagramManager{
	...
	public void createUCMDiagram(UCMModel model) {
		...
		for (int i = 0; i < model.getPaths().size(); i++) {
			...
			for (int j = 0; j < path.getNodes().size(); j++) {
				...
				ucmDiagrams.addNote(pathNode.getResponsibilityName(), pathNode.getComment());
	...
	public void createComponentDiagram(ComponentModel model){
		...
		for (int i = 0; i < componentsModel.size(); i++) {
			...
			for (int j = 0; j < ports.size(); j++) {
				...
				if (port.hasProvidedInterface())
					componentsDiagram.createProvidedInterface(port.getProvided().getName(), port.getName(), port.getComponent().getName());
				if (port.hasRequiredInterface())
					componentsDiagram.createRequiredsInterface(port.getRequired().getName(), port.getName(), port.getComponent().getName());
			}
			componentsDiagram.setScope(c.getName(), model.getAssociations(c.getName()));
		}
		...
		for (int i = 0; i < interfaceLinksModel.size(); i++) {
			...
			String componentSource = interfaceLink.getSource().getPort().getComponent().getName();
			String componentTarget = interfaceLink.getTarget().getPort().getComponent().getName();
			componentsDiagram.createInterfaceLink(	componentSource, 
													componentTarget, 
													interfaceLink.getSource().getPort().getName(),
													interfaceLink.getTarget().getPort().getName(),
													interfaceLink.getSource().getName(),
		...
		for (int i = 0; i < relationships.size(); i++) {
			...
			componentsDiagram.createRelationship(relation.getSource().getName(), relation.getTarget().getName(), relation.getStereotype());
		}
		...
		for (int i = 0; i < responsibilities.size(); i++) {
			...
			if (responsibility.hasComponent()){
				componentsDiagram.registryResponsibility(	responsibility.getComponent().getName(),
															responsibility.getName());
				
				componentsDiagram.mappingResponsibility(	responsibility.getName(), 
															model.getMapping(responsibility.getName()));
			}
		}
	...
	public ComponentsDiagram getComponentsDiagram(String name){
		...
		for (int i = 0; i < components.size(); i++) {
			ComponentsDiagram componentsDiagra = (ComponentsDiagram) components.get(i);
	...
	public void update(boolean componentDiagram){
		...
		if (editorPart != null){
			...
			if (componentDiagram){
				ComponentsDiagram c = (ComponentsDiagram) components.get(currentComponent-1);
				flabotFileModel.getDiagrams().add(c.getDiagram());
				flabotFileModel.getOpenDiagrams().add(c.getDiagram());
				editorPart.openDiagram((Diagram)flabotFileModel.getDiagrams().get(currentComponent-1));
			}
			else{
				...
				editorPart.openDiagram((Diagram)flabotFileModel.getDiagrams().get(currentComponent + currentUCM-1));
			}
	...
	private void save(){
		...
		try {
			resource.save(Collections.singletonMap(XMLResource.OPTION_ENCODING, "ISO-8859-15"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	private FlabotMultiPageEditor getFlabotEditorPart() {
		IWorkbenchPage[] workbenchPageList = DiagramPlugin.getDefault().getWorkbench().getActiveWorkbenchWindow().getPages();	
		for (int i = 0; i < workbenchPageList.length; i++) {
			...
			for (int j = 0; j < editorPartList.length; j++) {
				...
				if (editorPart instanceof FlabotMultiPageEditor){
					//if (((FlabotMultiPageEditor) editorPart).getPartName().equals(fileName)){//bug
					if (((FlabotMultiPageEditor) editorPart).getPartName().endsWith(".flabot")){
	...
	public void addNote(String description){
		if (0 < components.size() ){
			ComponentsDiagram componentsDiagram = (ComponentsDiagram) components.get(components.size()-1);
