	...
	public void run(){
		...
		for(Iterator ir=responsibilities.iterator();ir.hasNext();){
			...
			for(Iterator im=materializations.iterator();im.hasNext();){
				...
				String execId= ((PropertyLogNode)mat.getTags().get(LogSearcher.EXEC_ID_PROPERTY)).getValue();
				s=predicateFactory.createExecutionPredicate(res.getName(),execId);
				factsList.add(s);
				{	
					TagLogNode behTag= (TagLogNode)mat.getTags().get(LogSearcher.BEHAVIOR_TAG);
					InnerTag behavior=searcher.getTagLogNodeInfo(behTag);
					String behavior_name=((PropertyLogNode)behavior.getTags().get(LogSearcher.ALL_NAME_PROPERTY)).getValue();
					s=predicateFactory.createExecutionMethodPredicate(execId,behavior_name);
					factsList.add(s);
					{
						TagLogNode decClassTag=(TagLogNode)behavior.getTags().get(LogSearcher.BEHAVIOR_DECLARING_CLASS_TAG);
						InnerTag declaringClass=searcher.getTagLogNodeInfo(decClassTag);
						{
							String className= ((PropertyLogNode)declaringClass.getTags().get(LogSearcher.ALL_NAME_PROPERTY)).getValue();
